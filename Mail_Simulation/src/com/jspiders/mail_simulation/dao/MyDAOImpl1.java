package com.jspiders.mail_simulation.dao;
...
import com.jspiders.mail_simulation.model.MailDTO;
import com.jspiders.mail_simulation.model.UserDTO;
@Component
public class MyDAOImpl1 implements MyDAO{
	...
	public List<MailDTO> sentitem() {
		...
		Criterion oneCondition = 
			    Restrictions.conjunction().add(status).add(sentby).add(status2);
		Criterion completeCondition = 
		    Restrictions.disjunction().add(oneCondition).add(status1);
		cr1.add(completeCondition);
		List<MailDTO> mlist=cr1.list();
		ss.close();
		return mlist;
	...
	public MailDTO composedraft(int id) {
		...
		Criteria cr=ss.createCriteria(MailDTO.class);
		cr.add(Restrictions.eq("Id", id));
		MailDTO m=(MailDTO) cr.uniqueResult();
		ss.close();
		return m;
	}
